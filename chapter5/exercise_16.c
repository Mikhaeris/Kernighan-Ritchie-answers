...
int my_alphstr_cmp(char *, char *);
int my_dirorder_cmp(char *, char *);
int my_dirorder_alph_cmp(char *, char *);

int my_num_cmp(char *, char *);
int my_revnum_cmp(char *s1, char *s2);
...
int main(int argc, char *argv[]) {
    ...
    if ((nlines = readlines(lineptr, MAX_LINES)) >= 0) {
        ...
        if (recursive && numeric) {
            ...
            pcomp = my_num_cmp;
        } else if (alph_register && directory_order){
            pcomp = my_dirorder_alph_cmp;
        } else if (alph_register) {
            pcomp = my_alphstr_cmp;
        } else if (directory_order) {
            pcomp = my_dirorder_cmp;
        } else {
            pcomp = my_str_cmp;
        }
...
int my_alphstr_cmp(char *s, char *t) {
...
int my_dirorder_cmp(char *s, char *t) {
    for ( ; ; s++, t++) {
        while (!(my_isalnum(*s) || *s == ' ') && *s != '\0') {
            s++;
        }

        while (!(!my_isalnum(*t) || *t == ' ') && *t != '\0') {
            t++;
        }
...
int my_dirorder_alph_cmp(char *s, char *t) {
    for ( ; ; s++, t++) {
        while (!(my_isalnum(*s) || (*s == ' ')) && *s != '\0') {
            s++;
        }
...
int my_num_cmp(char *s1, char *s2) {
    double v1 = atof(s1);
    double v2 = atof(s2);
